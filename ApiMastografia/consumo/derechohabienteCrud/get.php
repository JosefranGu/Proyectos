    ...
    <div class="row justify-content-center border border-5 p-5 border-dark">
      <div class="col-md-12">
        <table class="table table-bordered table-striped ">
          ...
          <tbody>
            <?php
            $json_string = file_get_contents("http://localhost:8080/ApiMastografia/derechohabiente/read.php");
            $data = json_decode($json_string);
            $derechohabientes = $data->Derechohabiente;
            foreach ($derechohabientes as $dato) {
            ?>
              <tr>
                <td><?php echo $dato->id_deha; ?></td>
                <td><?php echo $dato->curp_deha; ?></td>
                <td><?php echo $dato->nombre_deha; ?></td>
                <td><?php echo $dato->tel_deha; ?></td>
                <td><?php echo $dato->correo_deha; ?></td>
                <td><button><a class="btn btn-warning" href="edit.php?id=<?php echo $dato->id_deha; ?>">Editar</a></button></td>
                <td><button><a class="btn btn-danger" href="delete.php?id=<?php echo $dato->id_deha; ?>">Eliminar</a></button></td>
              </tr>
            <?php
            }
            ?>
