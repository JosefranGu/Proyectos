<?php
    ...
    $sentencia = $bd->prepare("UPDATE bano SET Fecha = ? ,NombreCompleto = ? ,Telefono = ? ,Adicional = ? ,NumArticulos = ? ,Articulo = ? ,Monto = ? ,Resta = ? ,Pagado = ? ,Entregado = ? ,Total = ? ,Recibo1 = ? , Nota= ?  where Recibo = ?;");
    ...
    if ($resultado === TRUE) {
        header('Location: listarBaño.php?mensaje=editado');
    } else {
        header('Location: listarBaño.php?mensaje=error');
        exit();
    }
