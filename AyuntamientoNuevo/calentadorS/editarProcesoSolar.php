<?php
    ...
    $sentencia = $bd->prepare("UPDATE solar SET Fecha = ? ,NombreCompleto = ? ,Telefono = ? ,Adicional = ? ,NumArticulos = ? ,Articulo = ? ,Monto = ? ,Resta = ? ,Pagado = ? ,Entregado = ? ,Total = ? ,Recibo1 = ? , Nota= ?  where Recibo = ?;");
    ...
    if ($resultado === TRUE) {
        header('Location: listarSolar.php?mensaje=editado');
    } else {
        header('Location: listarSolar.php?mensaje=error');
        exit();
    }
