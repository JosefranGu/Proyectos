<?php
    ...
    $sentencia = $bd->prepare("UPDATE fumigadoras SET Fecha = ? ,NombreCompleto = ? ,Telefono = ? ,Adicional = ? ,NumArticulos = ? ,Articulo = ? ,Monto = ? ,Resta = ? ,Pagado = ? ,Entregado = ? ,Total = ? ,Recibo1 = ? , Nota= ?  where Recibo = ?;");
    ...
    if ($resultado === TRUE) {
        header('Location: listarF.php?mensaje=editado');
    } else {
        header('Location: listarF.php?mensaje=error');
        exit();
    }
