<?php
    ...
    $sentencia = $bd->prepare("UPDATE laminas SET Fecha = ? ,NombreCompleto = ? ,Telefono = ? ,Adicional = ? ,NumArticulos = ? ,Articulo = ? ,Monto = ? ,Resta = ? ,Pagado = ? ,Entregado = ? ,Total = ? ,Recibo1 = ? , Nota= ?  where Recibo = ?;");
    ...
    if ($resultado === TRUE) {
        header('Location: listarLamina.php?mensaje=editado');
    } else {
        header('Location: listarLamina.php?mensaje=error');
        exit();
    }
