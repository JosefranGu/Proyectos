<?php
    ...
    $sentencia = $bd->prepare("UPDATE parihuela SET Fecha = ? ,NombreCompleto = ? ,Telefono = ? ,Adicional = ? ,NumArticulos = ? ,Articulo = ? ,Monto = ? ,Resta = ? ,Pagado = ? ,Entregado = ? ,Total = ? ,Recibo1 = ? , Nota= ?  where Recibo = ?;");
    ...
    if ($resultado === TRUE) {
        header('Location: listarParihuela.php?mensaje=editado');
    } else {
        header('Location: listarParihuela.php?mensaje=error');
        exit();
    }
