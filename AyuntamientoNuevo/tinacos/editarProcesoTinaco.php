<?php
    ...
    $sentencia = $bd->prepare("UPDATE tinacos SET Fecha = ? ,NombreCompleto = ? ,Telefono = ? ,Adicional = ? ,NumArticulos = ? ,Articulo = ? ,Monto = ? ,Resta = ? ,Pagado = ? ,Entregado = ? ,Total = ? ,Recibo1 = ? , Nota= ?  where Recibo = ?;");
    ...
    if ($resultado === TRUE) {
        header('Location: listarTinaco.php?mensaje=editado');
    } else {
        header('Location: listarTinaco.php?mensaje=error');
        exit();
    }
