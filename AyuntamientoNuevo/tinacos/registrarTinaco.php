<?php
    ...
    if(empty($_POST["oculto"]) || empty($_POST["txtRecibo"]) || empty($_POST["txtFecha"]) || empty($_POST["txtNombre"] || empty($_POST["txtTelefono"])) || empty($_POST["txtTelefonoAdi"])|| empty($_POST["txtNumArt"])|| empty($_POST["txtDescripcion"])|| empty($_POST["txtMonto"]|| empty($_POST["txtResta"])|| empty($_POST["txtPagado"]) || empty($_POST["txtEntregado"])) || empty($_POST["txtTotal"]) || empty($_POST["txtNumRecibo"]) || empty($_POST["txtNota"])){
        header('Location: registroTinaco.php?mensaje=falta');
        exit();
    }
    ...
    $sentencia = $bd->prepare("INSERT INTO tinacos (Recibo,Fecha,NombreCompleto,Telefono,Adicional,NumArticulos,Articulo,Monto,Resta,Pagado,Entregado,Total,Recibo1,Nota) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
    ...
    if ($resultado === TRUE) {
        header('Location: registroTinaco.php?mensaje=registrado');
    } else {
        header('Location: registroTinaco.php?mensaje=error');
        exit();
    }
