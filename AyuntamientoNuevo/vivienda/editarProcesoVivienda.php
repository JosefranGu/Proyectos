<?php
    ...
    $sentencia = $bd->prepare("UPDATE vivienda SET Fecha = ? ,NombreCompleto = ? ,Telefono = ? ,Adicional = ? ,NumArticulos = ? ,Articulo = ? ,Monto = ? ,Resta = ? ,Pagado = ? ,Entregado = ? ,Total = ? ,Recibo1 = ? , Nota= ?  where Recibo = ?;");
    ...
    if ($resultado === TRUE) {
        header('Location: listarVivienda.php?mensaje=editado');
    } else {
        header('Location: listarVivienda.php?mensaje=error');
        exit();
    }
